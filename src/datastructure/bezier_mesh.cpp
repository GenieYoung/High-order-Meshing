...
int BezierMesh<T>::addVertex(const Vec2<T>& v)
{
    if (vertex2index.find(v) == vertex2index.end())
    {
        ...
        allVerticesWeights.resize(allVertices.size(), T(1.0));
    }
...
int BezierMesh<T>::addCtrlpt(const Vec2<T>& v)
{
    if (ctrlpt2index.find(v) == ctrlpt2index.end())
    {
        ...
        allCtrlptsWeights.resize(allCtrlpts.size(), T(1.0));
    }
...
void BezierMesh<T>::fillNeighbourInfo()
{
    ...
    for( Vec2<T> p: allCtrlpts )
        allPts.push_back(p);
    allWts.clear();
    for (T p : allVerticesWeights)
        allWts.push_back(p);
    for (T p : allCtrlptsWeights)
        allWts.push_back(p);
...
template <typename T>
double BezierMesh<T>::bz_get_c_uv_ijk(int ui, int i, int j, int k)
{
    if (i < 0 || j < 0 || k < 0)
        return 0.0;

    return C_UV_IJK[ui][bz_control_point_local_id(i, j, k)];
}
...
std::vector<Jacobian> BezierMesh<T>::bz_jacobian(std::vector< Vec2<T> >& ctrPt, std::vector<int>& t_ctrl_pt)
{
    ...
    for (unsigned int i = 0; i < t_ctrl_pt.size(); ++i)
    {
        if (allWts[t_ctrl_pt[i]] != 1.0)
        {
            isRational = true;
            break;
        }
    ...
    {
        double xw_u = 0.0, xw_v = 0.0;
        double yw_u = 0.0, yw_v = 0.0;
        double w_u = 0.0, w_v = 0.0;
        ...
        for (int cp = 0; cp < m_control_point_count; cp++)
        {
            ...
            double wz = utilCast<T, double>(allWts[t_ctrl_pt[cp]]);

            double bz_vali = bz_get_uv_ijk(ui, i - 1, j, k);
            double bz_valj = bz_get_uv_ijk(ui, i, j - 1, k);
            double bz_valk = bz_get_uv_ijk(ui, i, j, k - 1);
            double bz_ijk = bz_get_c_uv_ijk(ui, i, j, k);

            xw_u += bz.x * wz * (bz_vali - bz_valk);
            yw_u += bz.y * wz * (bz_vali - bz_valk);
            xw_v += bz.x * wz * (bz_valj - bz_valk);
            yw_v += bz.y * wz * (bz_valj - bz_valk);
            w_u += wz * (bz_vali - bz_valk);
            w_v += wz * (bz_valj - bz_valk);
            xw += bz.x * wz * bz_ijk;
            yw += bz.y * wz * bz_ijk;
            ww += wz * bz_ijk;
        }
        ...
        if (isRational)
        {
            j_local.a = (xw_u * ww - xw * w_u) / (ww * ww);
            j_local.b = SQRT3_INV * (-j_local.a + 2.0 * (xw_v * ww - xw * w_v) / (ww * ww));
            j_local.c = (yw_u * ww - yw * w_u) / (ww * ww);
            j_local.d = SQRT3_INV * (-j_local.c + 2.0 * (yw_v * ww - yw * w_v) / (ww * ww));
        }
        else
        {
            j_local.a = xw_u;
            j_local.b = SQRT3_INV * (-xw_u + 2.0 * xw_v);
            j_local.c = yw_u;
            j_local.d = SQRT3_INV * (-yw_u + 2.0 * yw_v);
        }
        J.push_back(j_local);
    }
    return J;
    ...
    for(unsigned int ui=0; ui<uv_sampling.size(); ui++)
    {
        ...
        for(int i=0; i<=degree; i++)
        {
            for(int j=0; i+j<=degree; j++)
            {
                ...
                double wij = utilCast<T, double>(allWts[id]);
                ...
                if (isRational)
                {
                    d_a = ((bz_vali - bz_valk) * wij * ww - bz_ijk * wij * w_u) / (ww * ww);
                    d_b = -SQRT3_INV * d_a + 2.0 * SQRT3_INV * ((bz_valj - bz_valk) * wij * ww - bz_ijk * wij * w_v) / (ww * ww);
                }
                else
                {
                    d_a = (bz_vali - bz_valk);
                    d_b = -SQRT3_INV * (bz_vali - bz_valk) + 2.0 * SQRT3_INV * (bz_valj - bz_valk);
                }
...
bool BezierMesh<T>::check_flip(vector< Vec2<T> >& ctrPt, std::vector<int>& f_ctrPt)
{
    ...
    std::vector<T> xx, yy,w;
    for (int p : f_ctrPt)
    {
        xx.emplace_back(ctrPt[p].x);
        yy.emplace_back(ctrPt[p].y);
        w.emplace_back(allWts[p]);
    }
    int result = ck.is_definitely_injective_by_determinant(xx, yy, w, true);
    if(1 == result)
        return false;
    return true;
}
...
void BezierMesh<T>::optimization_conformal()
{
    ...
    for(unsigned int i=0; i<uv_sampling.size(); i++)
    {
        ...
        std::vector<double>c_uv_ijk;
        for (unsigned int cp = 0; cp < C_IJK.size(); cp++)
        {
            int i = C_IJK[cp].first;
            int j = C_IJK[cp].second;
            int k = degree - i - j;
            c_uv_ijk.push_back(bz_value(degree, i, j, k, u, v));
        }
        C_UV_IJK.emplace_back(c_uv_ijk);
    }
