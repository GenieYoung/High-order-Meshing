...
bool bezierMeshing(std::string filename, bool addBbox, bool epsOutput, bool optimizeMesh, int exp_type)
{
#ifdef ALL_EXACT
    ...
    {
        if (!curve.isRconic() && !curve.isRcubic() && curve.degree() != 1)
    ...
    for (uint i = 0; i < guardedCurves.size(); i++)
    {
        ...
        guardedCurves[i].m_curve.TransformW();
