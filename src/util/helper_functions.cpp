...
bool checkMesh(BezierMesh<T>& mesh, bool& uncertain)
{
    ...
    for (const Triangle& t : mesh.allTriangles)
    {
        ...
        for (int vindex : t.vertices)
        {
            ...
            weightsRecursive.emplace_back(mesh.allVerticesWeights[vindex]);
        }
