...
template <typename T_IN, typename T_OUT> vector<T_OUT> convertStdVec(const vector<T_IN>& v)
{
    vector<T_OUT>result;
    for (const auto& e : v)
    {
        result.emplace_back(utilCast<T_IN, T_OUT>(e));
    }
    return result;
}
...
template <typename T_IN, typename T_OUT> BezierCurve<T_OUT> convertCurve(const BezierCurve<T_IN>& c)
{
    ...
    vector<T_OUT>weightsConverted;
    for (const Vec2<T_IN>& pt : c.m_ctrlpts)
    {
        ctrlptsConverted.emplace_back(convertVec<T_IN, T_OUT>(pt));
    }
    for (const T_IN& w : c.m_weights)
    {
        weightsConverted.emplace_back(utilCast<T_IN, T_OUT>(w));
    }
