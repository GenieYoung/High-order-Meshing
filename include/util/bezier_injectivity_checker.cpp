...
void BezierTriangleInjectivityChecker<T>::set_degree(int degree)
{
    ...
    indRC = std::vector<int>(degreeRC + 1);
    val = 0;
    add = degreeRC + 1;
    for (int i = 0; i <= degreeRC; i++)
    {
        indRC[i] = val;
        val += add;
        add--;
...
template <typename T>
int BezierTriangleInjectivityChecker<T>::is_definitely_injective_by_determinant(const std::vector<T>& _b1,
    ...
    bool via_bisect = true)
{
    ...
    if (w == std::vector<T>(w.size(), T(1.0)))
    {
        return is_definitely_injective_by_determinant(v1, v2, via_bisect);
    }
    ...
    std::vector<T>x_u = multiply(w, derivate_u(xw), degreeB, degreeB - 1) - multiply(xw, derivate_u(w), degreeB, degreeB - 1);
    std::vector<T>x_v = multiply(w, derivate_v(xw), degreeB, degreeB - 1) - multiply(xw, derivate_v(w), degreeB, degreeB - 1);
    std::vector<T>y_u = multiply(w, derivate_u(yw), degreeB, degreeB - 1) - multiply(yw, derivate_u(w), degreeB, degreeB - 1);
    std::vector<T>y_v = multiply(w, derivate_v(yw), degreeB, degreeB - 1) - multiply(yw, derivate_v(w), degreeB, degreeB - 1);
    std::vector<T>x_u_y_v = multiply(x_u, y_v, 2 * degreeB - 1, 2 * degreeB - 1);
    std::vector<T>x_v_y_u = multiply(x_v, y_u, 2 * degreeB - 1, 2 * degreeB - 1);
    std::vector<T>C = x_u_y_v - x_v_y_u;
    ...
    while (!q.empty())
    {
        ...
        T mic
            = std::min(C[indRC[degreeRC] + 0],
                std::min(C[indRC[0] + degreeRC], C[indRC[0] + 0])); // minimum of corner values
        if (mic <= min_area)
        {
            return 0; // determinant non-positive at some (corner) point
        }
        ...
        else
        {
            ...
            for (int i = 0; i <= degreeRC; i++)
                for (int m = 0; m <= i; m++)
                    C0.push_back(P(C, i, 0, m, 0, degreeRC - i));
            for (int i = 0; i <= degreeRC; i++)
                for (int m = 0; m <= degreeRC - i; m++)
                    C1.push_back(P(C, i, degreeRC - i, i, m, 0));
            for (int i = 0; i <= degreeRC; i++)
                for (int m = 0; m <= degreeRC - i; m++)
                    C2.push_back(P(C, 0, i, 0, i, m));
            for (int i = 0; i <= degreeRC; i++)
                for (int j = 0; j <= degreeRC - i; j++)
                    C3.push_back(P(C, i, j, i, j, degreeRC - i - j));
