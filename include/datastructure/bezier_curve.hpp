...
template <typename T> struct BezierCurve : public Curve<T>
{
    ...
    BezierCurve(const vector<Vec2<T>>& ctrlpts, const vector<T>& weights, T tStart = 0, T tEnd = 1, int id = 0)
        : Curve<T>(ctrlpts, weights, tStart, tEnd, id)
    {
    }
    ...
    void TransformW()
    {
        if (isRconic())
        {
            m_weights[1] = m_weights[1] * utilCast<double, T>(std::sqrt(1 / (utilCast<T, double>(m_weights[0]) * utilCast<T, double>(m_weights[2]))));
            m_weights[0] = T(1.0);
            m_weights[2] = T(1.0);
        }
        else if (isRcubic())
        {
            m_weights[1] = m_weights[1] * utilCast<double, T>((1.0 / std::pow(utilCast<T, double>(m_weights[3]), 1.0 / 3)) * (1.0 / std::pow(utilCast<T, double>(m_weights[0]), 2.0 / 3)));
            m_weights[2] = m_weights[2] * utilCast<double, T>((1.0 / std::pow(utilCast<T, double>(m_weights[3]), 2.0 / 3)) * (1.0 / std::pow(utilCast<T, double>(m_weights[0]), 1.0 / 3)));
            m_weights[0] = T(1.0);
            m_weights[3] = T(1.0);
        }
    }
    ...
    bool isRconic()const
    {
        return this->degree() == 2 && m_weights != vector<T>(3, T(1.0));
    }

    bool isRcubic()const
    {
        return this->degree() == 3 && m_weights != vector<T>(4, T(1.0));
    }
    ...
    virtual Vec2<T> operator()(const T& t) const
    {
        ...
        {
            return (m_ctrlpts[0] * (T(1.0) - t) * (T(1.0) - t) * m_weights[0] + m_ctrlpts[1] * 2 * t * (T(1.0) - t) * m_weights[1] + m_ctrlpts[2] * t * t * m_weights[2])
                / ((T(1.0) - t) * (T(1.0) - t) * m_weights[0] + 2 * t * (T(1.0) - t) * m_weights[1] + t * t * m_weights[2]);
        }
        else
        {
            return (m_ctrlpts[0] * (T(1.0) - t) * (T(1.0) - t) * (T(1.0) - t) * m_weights[0] + m_ctrlpts[1] * 3 * (T(1.0) - t) * (T(1.0) - t) * t * m_weights[1] + m_ctrlpts[2] * 3 * (T(1.0) - t) * t * t * m_weights[2] + m_ctrlpts[3] * t * t * t * m_weights[3])
                / ((T(1.0) - t) * (T(1.0) - t) * (T(1.0) - t) * m_weights[0] + 3 * (T(1.0) - t) * (T(1.0) - t) * t * m_weights[1] + 3 * (T(1.0) - t) * t * t * m_weights[2] + t * t * t * m_weights[3]);
        }
    }
    ...
    bool increaseDegree(uint delta)
    {
        ...
        if (!isRconic() && !isRcubic())
        {
            while (delta-- > 0)
            {
                ...
                m_ctrlpts = ctrlpts;
                m_weights = vector<T>(m_ctrlpts.size(), T(1.0));
            }
            return true;
        }
        else
        {
            while (delta-- > 0)
            {
                vector<Vec2<T>> ctrlpts = { m_ctrlpts[0] * m_weights[0] };
                vector<T>weights = { m_weights[0] };
                for (unsigned int i = 1; i < m_ctrlpts.size(); i++)
                {
                    Vec2<T> pt = (m_ctrlpts[i - 1] * m_weights[i - 1] * T(i) + m_ctrlpts[i] * m_weights[i] * T(degree() + 1 - i))
                        / T(degree() + 1);
                    T wt = (m_weights[i - 1] * T(i) + m_weights[i] * (T)(degree() + 1 - i)) / T(degree() + 1);
                    pt = pt / wt;
                    ctrlpts.emplace_back(pt);
                    weights.emplace_back(wt);
                }
                ctrlpts.emplace_back(m_ctrlpts[degree()]);
                weights.emplace_back(m_weights[degree()]);
                m_ctrlpts = ctrlpts;
                m_weights = weights;
            }
            return true;
