...
template <typename T> struct Curve
{
    ...
    Curve(const vector<Vec2<T>>& ctrlpts, T tStart = 0, T tEnd = 1, int id = 0)
        : m_tStart(tStart), m_tEnd(tEnd), m_origTStart(tStart), m_origTEnd(tEnd), m_id(id),
          m_origId(id), m_ctrlpts(ctrlpts), m_weights(ctrlpts.size(), T(1.0)), m_origLen(-1.0)
    {
        if (ctrlpts.size() < 2)
        {
    ...
    Curve(const vector<Vec2<T>>& ctrlpts, const vector<T>& weights, T tStart = 0, T tEnd = 1, int id = 0)
        : m_tStart(tStart), m_tEnd(tEnd), m_origTStart(tStart), m_origTEnd(tEnd), m_id(id),
        m_origId(id), m_ctrlpts(ctrlpts), m_weights(weights), m_origLen(-1.0)
    {
        if (ctrlpts.size() < 2)
        {
            Logger::lout(Logger::WARN) << "Curve with <2 control points created!" << endl;
